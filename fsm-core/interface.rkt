...
;; L(BIN-NUM) = all words representing binary numbers without leading 0s
;; Alphabet = {0 1}
(define BIN-NUMS
  (let* [;; L(ZERO) = {(0)}
          ;; Alphabet = {0}
          (ZERO (singleton-regexp "0"))

          ;; L(ONE) = {(1)}
          ;; Alphabet = {1}
          (ONE  (singleton-regexp "1"))

          ;; L(0U1) = {(0) (1)}
          ;; Alphabet = {0 1}
          (0U1 (union-regexp ZERO ONE))

          ;; L(0U1*) = all words with an arbitrary number of 0s and 1s
          ;; Alphabet = {0}
          (0U1* (kleenestar-regexp 0U1))

          ;; L(STARTS1) = all binary numbers starting with 1
          ;; Alphabet = {0 1}
          (STARTS1 (concat-regexp ONE 0U1*))]
