...
(module pda racket
  ...
  (provide make-ndpda rename-states-pda cfg->pda pda->cfg
  ...
  (define (make-ndpda K sigma gamma start finals pdarules)
      ...
      (cond [(null? tovisit) 'reject]
            [else
             (let* ((first-path (car tovisit))
                    ...
                    (accepts (get-pdaconfig-accepts new-configs w)))
               (if (not (null? accepts))
  ...
  (define (pda-getalphabet m) (m '() 'get-alphabet))
  ...
  (define (pda->spda m)
    ...
    (let* ((newS (gen-symbol 'S (pda-getstates m)))
           ...
           (delta (car res))
           (finalK (cdr res)))
      (make-ndpda finalK sigma gamma newS (list newF) delta)))
  ...
  (define (union-pda p1 p2)
    (let* ((newp2 (rename-states-pda (pda-getstates p2) p2))
      ...
      (make-ndpda newStates newSigma newGamma newS newFinals newRules)))
    ...
    (let* ((newS (generate-symbol START (pda-getstates p1)))
           ...
           (newRules (append (list (list (list newS EMP EMP) (list (pda-getstart p1) EMP))) 
                             (map (lambda (s)(list (list s EMP EMP) (list (pda-getstart p1) EMP))) 
                                  (pda-getfinals p1))
                             (pda-getrules p1))))
      (make-ndpda newStates (pda-getalphabet p1) (pda-getgamma p1) newS newFinals newRules)))
