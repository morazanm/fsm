...
(unsafe-require/typed racket/hash
                      ...
                      #;[hash-union! (->* ((Mutable-HashTable Symbol String) (HashTable Symbol String)) (#:combine (-> String String String)
                                                                                                         #:combine/key (-> Symbol String String String)) Void)])
...
(struct formatters ([graph : (Mutable-HashTable Symbol (-> Any String))]
                    [node : (Mutable-HashTable Symbol (-> Any String))]
                    [edge : (Mutable-HashTable Symbol (-> Any String))]) #:transparent)
...
#;(: hash->rope (->* ((Mutable-HashTable Symbol Any) (Mutable-HashTable Symbol (-> Any String))) (String) rope))
#;(define (hash->rope hash fmtr (spacer ", "))
  ...
  (: key-val->rope (-> Symbol Any rope))
  (define (key-val->rope key value)
    ;(: fmtr-fun (U Procedure Boolean) : Procedure)
    (define fmtr-fun (hash-ref fmtr key #f))
    (string->rope (if (procedure? fmtr-fun)
                      (format "~s=~s" key (fmtr-fun value))
                      (format "~s=~s" key (if (equal? key 'label)
                                              (format "~a" (fmt-val value))
                                              (fmt-val value)
  ...
  (rope-join (hash-map hash key-val->rope) (string->rope spacer))
  )
...
#;(: node->rope (-> node (Mutable-HashTable Symbol (-> Any String)) rope))
#;(define (node->rope node fmtr-node)
  #;(println (format "hash->str: ~a" (hash->str (node-atb node) fmtr-node)))
  #;(println (format "node name: ~a" (symbol->string (node-name node))))
  #;(println (format "rope to str: ~a" (rope->string (rope-append* (string->rope "    ")
                                                     (string->rope (symbol->string (node-name node)))
                                                     (string->rope " [")
                                                     (string->rope (hash->str (node-atb node) fmtr-node))
                                                     (string->rope "];\n")))))
  (rope-append* (string->rope "    ")
                (string->rope (symbol->string (node-name node)))
                (string->rope " [")
                (hash->rope (node-atb node) fmtr-node)
                (string->rope "];\n"))
  #;(string->rope (format "    ~s [~a];\n"
            (node-name node)
            (hash->rope (node-atb node) fmtr-node))))
...
#;(: edge->rope (-> edge (Mutable-HashTable Symbol (-> Any String)) rope))
#;(define (edge->rope edge fmtr)
  (rope-append* (string->rope "    ")
                (string->rope (symbol->string (edge-start-node edge)))
                (string->rope " -> ")
                (string->rope (symbol->string (edge-end-node edge)))
                (string->rope " [")
                (hash->rope (edge-atb edge) fmtr)
                (string->rope "];\n"))
  #;(string->rope (format "    ~s -> ~s [~a];\n"
            (edge-start-node edge)
            (edge-end-node edge)
            (hash->str (edge-atb edge) fmtr))))
...
(define (subgraph->str sg fmtrs)
  ...
  (string-append (format "    subgraph ~s {\n" name) 
                 ...
                 (foldl (lambda ([n : node] [a : String]) (string-append a "    " (node->str n (formatters-node fmtrs))))
                        ""
                        (subgraph-node-list sg))
                 (foldl (lambda ([e : edge] [a : String]) (string-append a "    " (edge->str e (formatters-edge fmtrs))))
                        ""
...
#;(: graph->rope (-> graph rope))
#;(define (graph->rope g)
  (define name (format "digraph ~s {\n" (graph-name g)))
  (define fmtrs (graph-fmtrs g))
  (rope-append* (string->rope name)
                (string->rope (format "    ~a;\n" (hash->rope (graph-atb g) (formatters-graph fmtrs) ";\n    ")))
                (foldl (lambda ([n : node] [a : rope]) (rope-append a (node->rope n (formatters-node fmtrs))))
                       rope-empty
                       (graph-node-list g))
                (foldl (lambda ([e : edge] [a : rope]) (rope-append a (edge->rope e (formatters-edge fmtrs))))
                       rope-empty
                       (graph-edge-list g))
                (string->rope "}")
                )
...
(define (create-graph name #:fmtrs(fmtrs DEFAULT-FORMATTERS) #:atb [atb DEFAULT-GRAPH])
  (graph name
         ...
         '()
         (formatters
          (combine-formatters (formatters-graph fmtrs)
                              (formatters-graph DEFAULT-FORMATTERS))
          (combine-formatters (formatters-node fmtrs)
                              (formatters-node DEFAULT-FORMATTERS))
          (combine-formatters (formatters-edge fmtrs)
      ...
      [(not edge-index)
       ...
       (cons (edge (if (and (cons? is-subgraph-start?)
                   ...
                   (let [
                     ...
                     (hash-set! new-atb 'label (list val))
...
;; NOTE: a edges label is a list since we squash all edges between the same nodes
;; into a single edge
(: add-edges (->* ((U graph subgraph) (Listof (List Symbol Symbol Symbol))) (#:atb (Mutable-HashTable Symbol Any)) (U graph subgraph)))
(define (add-edges parent edgs #:atb [atb DEFAULT-EDGE])
  (foldl (lambda ([e : (List Symbol Symbol Symbol)] [a : (U graph subgraph)])
...
#;(: rope-graph->dot (-> graph Path String Path))
#;(define (rope-graph->dot graph save-dir filename)
  (define dot-path (build-path save-dir (format "~a.dot" filename)))
  (call-with-output-file dot-path
    #:exists 'replace
    (lambda ([out : Output-Port])
      (displayln (rope->string (graph->rope graph)) out))
    )
  dot-path)
...
(: special-graph->str (-> graph (Listof Symbol) String))
(define (special-graph->str g rank-node-lst)
  ...
  (string-append
   ...
   (foldl (lambda ([e : subgraph] [a : String]) (string-append a (subgraph->str e fmtrs)))
          ""
          (graph-subgraph-list g))
   (format "    {rank=same;~a};\n" (foldr (lambda ([val : Symbol] [accum : String]) (string-append (symbol->string val) ";" accum)) "" rank-node-lst))
   "}"))

;; graph -> string
;; Returns graphviz representation of a graph containing a context free graph as a string
(: cfg-graph->str (-> graph (Listof (Listof Symbol)) String))
(define (cfg-graph->str g rank-node-lst)
  ...
  (string-append
   ...
   (foldl (lambda ([e : subgraph] [a : String]) (string-append a (subgraph->str e fmtrs)))
          ""
          (graph-subgraph-list g))
   (foldr (lambda ([lvl : (Listof Symbol)] [accum : String])
            (string-append (format "    {rank=same;~a};\n" (foldr (lambda ([val : Symbol] [accum : String]) (string-append (symbol->string val) ";" accum)) "" lvl)) accum))
          ""
          rank-node-lst)
   "}")
  )
...
;; graph -> string
;; Returns graphviz representation of a graph containing a context free graph as a string
#;(: cfg-graph->rope (-> graph (Listof (Listof Symbol)) rope))
#;(define (cfg-graph->rope g rank-node-lst)
  (define name (format "digraph ~s {\n" (graph-name g)))
  (define fmtrs (graph-fmtrs g))
  (let ([res (rope-append*
              (string->rope name)
              ...
              (foldl (lambda ([e : edge] [a : rope]) (rope-append a (edge->rope e (formatters-edge fmtrs))))
                     rope-empty
                     (graph-edge-list g))
              (foldr (lambda ([lvl : (Listof Symbol)] [accum : rope])
                       (rope-append (rope-append* (string->rope "    {rank=same;")
                                                  (foldr (lambda ([val : Symbol] [accum : rope]) (rope-append* (string->rope (symbol->string val))
                                                                                                                          (string->rope ";")
                                                                                                                          accum)) rope-empty lvl)
                                                  (string->rope "};\n")
                                     #;(format "    {rank=same;~a};\n"
                                                          (foldr (lambda ([val : Symbol] [accum : String]) (string-append (symbol->string val)
                                                                                                                          ";"
                                                                                                                          accum)) "" lvl))
                                                  )
                                    accum))
                     ...
                     rank-node-lst)
              (string->rope "}"))])
    #;(displayln (rope->string (foldl (lambda ([n : node] [a : rope]) (rope-append a (node->rope n (formatters-node fmtrs))))
                                    rope-empty
                                    (graph-node-list g))))
    res)
  )
...
(: special-graph->dot (-> graph (Listof Symbol) Path-String Path-String Path))
...
;; graph path string -> path
;; Writes graph containing context sensitive grammar to the specified file
(: cfg-graph->dot (-> graph (Listof (Listof Symbol)) Path-String Path-String Path))
...
#;(: rope-cfg-graph->dot (-> graph (Listof (Listof Symbol)) Path-String Path-String Path))
#;(define (rope-cfg-graph->dot graph rank-node-lst save-dir filename)
  (define dot-path (build-path save-dir (format "~a.dot" filename)))
  (call-with-output-file dot-path
    #:exists 'replace
    (lambda ([out : Output-Port])
      (displayln (rope->string (cfg-graph->rope graph rank-node-lst)) out)))
  dot-path)
