...
(define (get-list ctmd)
  (cases expression ctmd
    (ctmd-exp (l) l)
    (else '())))
               ...
               ((eqv? (car (car datum)) 'VAR)
                (var-exp
                 (if (pair? (cadr (car datum)))
                     ...
                     (cadr (car datum)))
                 (get-list (parse-program2 (cdr datum) 0.5 acclist)) #;(drop-right (cdr datum) 1)
  ...
  (cases expression exp
    ...
    #;(var-exp (var tms goto)
             (map (lambda (x i) (list (string-append (symbol->string (if (pair? x)
                  ...
                  (make-int-list tms 0)))
    #;(var-exp (var tms goto)
             (map (lambda (x) (node x 0)) (parse-program tms)))
    #;(var-exp (var tms goto)
             (var-nodes var tms goto))
    (else '())))
...
;; ctmd -> program
;; Purpose: Parse the given ctmd
(define (parse-program2 list int list2)
  (ctmd-exp
   (let* ((pl2
          (map (lambda (l i)
                 (parse-ctmd l i list2))
               (filter-list list2)
               (make-int-list list2 int)))
         (parsed-list
          (map (lambda (l i)
                 (parse-ctmd l i pl2))
               (filter-list list)
               (make-int-list (filter-list list) int))))
(begin (displayln parsed-list)
       (displayln pl2)
     (p parsed-list pl2)))))
...
;(filter-list SWAPL)
'(((VAR 'i) R ((VAR 'j) i L j)))
;(parse-program '(((VAR 'i) R ((VAR 'j) 'i L 'j))))
(parse-program '(((VAR 'i) R))) 



(parse-program COPYL2)
(clean-list (dot-nodes (parse-program COPYL2)))
(clean-list (dot-edges (parse-program FBRL)))
