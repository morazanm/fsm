               ...
               (if (= 1 (length (graph-upimgs (viz-state-graph a-vs))))
                   ...
                   (viz-state (graph (rest (graph-upimgs (viz-state-graph a-vs)))
                              ...
                              (above (make-tape-img (first (first new-ptape))
                                                    ...
                                                    (third (first new-ptape)))
                                     (square 30 'solid 'white)
                                     (if (or (equal? (first (var-pvar (viz-state-var a-vs))) '(label ""))
                                             (equal? (first (var-pvar (viz-state-var a-vs))) '(label "dummy"))
                                             (equal? (first (var-pvar (viz-state-var a-vs))) '(label "BLANK"))
                                             (tmconfig? (first (var-pvar (viz-state-var a-vs)))))
                                         (text "" 20 'black)
...
>>>>>>> Stashed changes
    ...
    (run-viz (viz-state (graph (rest loimgs) (list (first loimgs)))
                        ...
                        tapeimg
                        (var (rest lovars) (list (first lovars))))
             draw-world 'viz-ctm)))
