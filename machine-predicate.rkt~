...
"
            ...
            (define (valid-states? state-set)
              (local [(define invalid-states (filter (lambda (l) (or (equal? 'ds l)
                ...
                (cond [(empty? state-set) "the set of states cannot be empty"]
                      ...
                      [(and (list? the-set)
                            (not (empty? invalid-states))) (format "~s are not valid states and ~s are repeated values in the set ~s" invalid-states the-set state-set)]
                      [(list? the-set) (format "~s are repeated in the given state set~s " the-set state-set)]
                      [else (format "~s are not valid states" invalid-states)])))
            ...
            (define (valid-finals? F)
              (local [;check-final: state --> state/boolean
                ...
                (cond [(and (empty? invalid-finals)
                      ...
                      [(empty? invalid-finals) (format "~s are repeats in the set of final states and must be removed" repeats)]
                                  ...
                                  (local [(define invalid-sigmas (filter (lambda (x) (or (not (symbol? x))
                                    ...
                                    (cond [(and (equal? repeats true)
                                          ...
                                          [(and (not (equal? repeats true))
                                                (not (empty? invalid-sigmas))) (format "~s are invalid alphabet members and ~s are repeated values in the sigma, remove repeats" invalid-sigmas repeats)]
                                          [(equal? repeats true) (format "~s are invalid alphabet members" invalid-sigmas)]
                                          [else (format "~s are repeated values in the sigma, remove repeats" repeats)])))]
            ...
            (define (create-initials)
              (local [(define check1 (valid-start? s))
                ...
                (filter (lambda (x) (not (equal? x true))) (list check1 check2 check3 check4))))
            ...
            (define (valid-dfa? a-machine)
              (local [;no-dfa-repeats?: (listof (listof rules)) --> boolean
                      ...
                      (define big-error (filter (lambda (x) (not (equal? true x))) (cons start-err (cons (valid-delta-dfa? delta) initials))))]
