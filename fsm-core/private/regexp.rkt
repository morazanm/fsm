...
(require "constants.rkt"
         ...
         racket/set)
(provide null-regexp
         (rename-out [fsm-null-regexp? null-regexp?]
                     [fsm-empty-regexp? empty-regexp?])
         empty-regexp 
         singleton-regexp?
         singleton-regexp-a
         #;(contract-out
         ...
         (struct-out fsm-regexp)
         )
  
(define NULL-REGEXP-STRING "∅")
(define EMPTY-REGEXP-STRING (symbol->string EMP))

(struct fsm-regexp (alphabet) #:transparent)

(define (update-subregex-alphabet fsm-regex)
  (define whole-alphabet (fsm-regexp-alphabet fsm-regex))
  (define (recurse fsm-regex)
    (cond [(fsm-empty-regexp? fsm-regex) (empty-regexp whole-alphabet)]
          [(fsm-null-regexp? fsm-regex) (null-regexp whole-alphabet)]
          [(singleton-regexp? fsm-regex) (singleton-regexp whole-alphabet (singleton-regexp-a fsm-regex))]
          [(union-regexp? fsm-regex) (union-regexp whole-alphabet
                                                   (recurse (union-regexp-r1 fsm-regexp))
                                                   (recurse (union-regexp-r2 fsm-regexp)))]
          [(concat-regexp? fsm-regex) (concat-regexp whole-alphabet
                                                     (recurse (concat-regexp-r1 fsm-regexp))
                                                     (recurse (concat-regexp-r2 fsm-regexp)))]
          [(kleenestar-regexp? fsm-regex) (kleenestar-regexp whole-alphabet
                                                             (recurse (kleenestar-regexp-r1 fsm-regexp)))]))
  (recurse fsm-regex))

;   --> regexp
(struct fsm-null-regexp fsm-regexp () #:transparent) ;;; represents no transition
(define (null-regexp)
  (fsm-null-regexp (seteq)))

; --> empty-regexp
(struct fsm-empty-regexp fsm-regexp () #:transparent) ;;; represents empty transition
(define (empty-regexp)
  (fsm-empty-regexp (seteq)))

; single character string --> singleton-regexp
(struct singleton-regexp fsm-regexp (a) #:transparent)
...
(define (make-unchecked-singleton a)
  (singleton-regexp (seteq a) a)
  )
  
; regexp regexp --> concat-regexp
(struct concat-regexp fsm-regexp (r1 r2) #:transparent)
(define (make-unchecked-concat a b)
  (concat-regexp (set-union (fsm-regexp-alphabet a) (fsm-regexp-alphabet b)) a b))
  
; regexp regexp --> union-regexp
(struct union-regexp fsm-regexp (r1 r2) #:transparent)
(define (make-unchecked-union a b)
  (union-regexp (set-union (fsm-regexp-alphabet a) (fsm-regexp-alphabet b)) a b)
  )
  
; regexp --> kleenestar-regexp
(struct kleenestar-regexp fsm-regexp (r1) #:transparent)
(define (make-unchecked-kleenestar a)
  (kleenestar-regexp (fsm-regexp-alphabet a) a))
#|
...
(define (simplify-regexp x)
  ...
  (define (simplify-kleenestar x)
    ...
    (cond [(or (kleenestar-regexp? simp-x)
               (fsm-empty-regexp? simp-x)
               (fsm-null-regexp? simp-x))
           simp-x]
          [else (make-unchecked-kleenestar simp-x)]))
  ...
  (define (simplify-concat x)
    ...
    (cond [(or (fsm-null-regexp? simp-lhs)
               (fsm-null-regexp? simp-rhs))
           (fsm-null-regexp)]
          [(fsm-empty-regexp? simp-lhs) simp-rhs]
          [(fsm-empty-regexp? simp-rhs) simp-lhs]
          [else (make-unchecked-concat simp-lhs simp-rhs)]))
  ...
  (define (simplify-union x)
    ...
    (define (return-components x)
      (cond [(fsm-null-regexp? x) '()]
            [(or (fsm-empty-regexp? x)
                 (singleton-regexp? x)
                 (kleenestar-regexp? x)
                 (concat-regexp? x))
             (list x)]
    ...
    (define (re-union a-list)
      (cond [(null? a-list) (fsm-null-regexp)]
            [(null? (cdr a-list)) (car a-list)]
            [else (make-unchecked-union (car a-list) (re-union (cdr a-list)))]))
...
(define (printable-regexp r)
  ; regexp --> string
  (define (helper r)
    (cond [(fsm-null-regexp? r) NULL-REGEXP-STRING]
          [(fsm-empty-regexp? r) EMPTY-REGEXP-STRING]
...
(define (gen-regexp-word rexp . reps)
  ...
  (cond [(fsm-empty-regexp? rexp) EMP]
        ...
        [(fsm-null-regexp? rexp) (error "A word cannot be generated using the null-regexp.")]
