...
(define (check-invariant accept? inv words)
  (check-syntax (property-check valid-predicate?
                ...
                (property-check list?
                                ...
                                inv)
                (if accept?
                    (property-check (if (null? (val-stx-pair-val (car words)))
                                        (val-stx-pair-val inv)
                                        (if (list? (car (val-stx-pair-val (car words))))
                                            (if (and (not (null? (car (val-stx-pair-val (car words)))))
                                                     (not (null? (cdr (car (val-stx-pair-val (car words))))))
                                                     (null? (cdr (cdr (car (val-stx-pair-val (car words)))))))
                    ...
                    (property-check (let ([not-inv-func (compose1 not (val-stx-pair-val inv))])
                                      (if (null? (val-stx-pair-val (car words)))
                                          not-inv-func
                                          (if (list? (car (val-stx-pair-val (car words))))
                                              (if (and (not (null? (car (val-stx-pair-val (car words)))))
                                                       (not (null? (cdr (car (val-stx-pair-val (car words))))))
