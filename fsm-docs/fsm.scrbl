...
@(require (for-label racket 2htdp/image racket/undefined) setup/collects scribble/eval)
...
@defproc*[([(fsa-minimize [m dfa]) dfa]
          [(fsa-minimize [m ndfa]) dfa])]
          Builds a @italic{deterministic} finite-state 
 automaton with the minimal number of states.
...
@defproc*[([(minimization-viz [m dfa]) void?]
           [(minimization-viz [m ndfa]) void?])]
         Launches a visualization tool for the construction of a dfa with the minimal number of states that decides
         L = {w | w∈L(m)}.
...
@defproc[(sm-test-invs [m machine?] [#:rep-limit rep-limit natural? 1] [#:max-path-length path-length natural? 12]  [invs (list state (-> word boolean))] ...)
         (listof (list state word))]
        Tests the given machine using the given state invariants. Dead states are removed for testing purposes.
        If m is a finite-state machine, then @italic{rep-limit} is used to limit the number of times a transition is used.
        If m is a pda, then @italic{max-path-length} is used to limit the number of steps a computation can take. If
        all tests pass, then the function returns the empty list. Otherwise, the function returns a list of pairs,
        where each pair contains a word and the state for which the invariant failed.
...
@defproc[(sm-quickcheck [m machine?] [#:num-tests tests natural? 300] [#:max-length-kleenes num-kleenes natural? 20] [invs (list state (-> word boolean))] ...)
         (void)]
        Tests the given machine using the given state invariants. Dead states are removed for testing purposes.
        @italic{tests} is the number of tests to perform on each state. @italic{num-kleenes} is the maximum number of times
        a loop is traversed in the given machine's transition relation. If tests pass, the function returns void.
        If tests fail, failed test reports are generated. 
...
@defproc[(cfexp? [cfe cfexp])
         Boolean]{Predicate for context-free expressions.}

@defproc[(empty-cfexp? [cfe cfexp])
         Boolean]{Predicate for the empty context-free expression.}

@defproc[(singleton-cfexp? [cfe cfexp])
         Boolean]{Predicate for a singleton context-free expression.}

@defproc[(union-cfexp? [cfe cfexp])
         Boolean]{Predicate for a union context-free expression.}

@defproc[(concat-cfexp? [cfe cfexp])
         Boolean]{Predicate for a concat context-free expression.}

@defproc[(kleenestar-cfexp? [cfe cfexp])
         Boolean]{Predicate for a Kleene star context-free expression.}
@;{
