...
;; machine -> (listof (listof Rule))
;; Purpose: To return all the paths of the given machine that reach final states
(define (get-paths-to-finals machine-paths finals)
  (define finals-set (list->seteq finals))
  (define final-paths (mutable-seteq))
  (for*/list ([path (in-list machine-paths)]
              #:when (set-member? finals-set (third (last path))))
    (set-add! final-paths path))
  (set->list final-paths))


;;(listof (listof Rule)) -> (listof Rule)
;;Purpose: To extract the rules found in the paths to final states
(define (extract-rule-set paths-to-finals)
  (remove-duplicates (apply append paths-to-finals)))


;; (listof (listof Rule)) -> (listof Symbol)
;; Purpose: To extract the states from the given list of rules
(define (extract-state-set paths)
  (remove-duplicates (flatten (for*/list ([path (in-list paths)])
                                (list (first path) (third path))))))
              


;; (listof Rule) (listof State) -> boolean
;; Purpose: Determines if the given path uses rules for the states in the given states
(define (uses-only-these-states? lor los)
  (define states (list->set los))
  (andmap (λ (rule) (and (set-member? states (first rule))
                         (set-member? states (third rule))))
          lor))
                 
;; (listof paths) sethash -> (listof paths)
;; Purpose: Remove paths that don't use the given state 
(define (filter-paths machine-paths new-states)  
  (filter (λ (path) (uses-only-these-states? path new-states))
          machine-paths))

;; state (listof path) -> (listof path)
;; Purpose: Returns all the paths that go to the given state
(define (all-paths-to state lop)
  (filter (λ (path) (eq? (third (last path)) state))
          lop))
...
(define (get-all-regexp-new M)
  (define regexp-ht (make-hash))
  (define machine-paths (find-paths M))
  (define paths-to-finals
    (get-paths-to-finals machine-paths (sm-finals M)))
  (define new-rules (extract-rule-set paths-to-finals))
  (define new-states (extract-state-set new-rules))
  (define refactored-paths (filter-paths machine-paths new-states))
         
  (define paths-to-start-state
    (filter (λ (path) (eq? (third (last path)) (sm-start M)))
            refactored-paths))
  (define states-no-start-state (filter (λ (x) (not (eq? x (sm-start M)))) new-states))
  (if (empty? paths-to-start-state)
      (begin (hash-set! regexp-ht
                        (sm-start M)
                        (empty-regexp))
             (for ([A states-no-start-state])
               (let* [(A-paths (all-paths-to A refactored-paths))
                      (states4A (extract-state-set A-paths))
                      (rules4A (extract-rule-set A-paths))]
                 (hash-set! regexp-ht
                            A
                            (fsa->regexp
                             (make-unchecked-ndfa states4A
                                                  (sm-sigma M)
                                                  (sm-start M)
                                                  (list A)
                                                  rules4A))))))
