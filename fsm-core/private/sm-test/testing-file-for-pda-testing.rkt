...
(define (S-INV-aˆnbˆn ci stck)
  (and (= (length ci) (length stck))
       (andmap (λ (i g) (and (eq? i 'a) (eq? g 'b))) ci stck)))
...
;; A: ci = (a^nb^n)*a^n AND stack = Za^n
;; B: ci = (a^nb^n)*a^mb^l AND stack = Za^n-l
...
(define (A-INV-a^nb^n* ci stack)
  (let* ([B-stack (takef-right stack (λ (x) (eq? 'Z x)))]
...
;; tests for a-inv
;(check-equal? (A-INV-a^nb^n* '() '(Z)) #t)
;(check-equal? (A-INV-a^nb^n* '(a) '(a Z)) #t)
;(check-equal? (A-INV-a^nb^n* '(a a b b a) '(a Z)) #t)
;(check-equal? (A-INV-a^nb^n* '(a b a a) '(a a Z)) #t)
...
;; tests for b-inv
;(check-equal? (B-INV-a^nb^n* '(a b) '(Z)) #t)
;(check-equal? (B-INV-a^nb^n* '(a a b b) '(Z)) #t)
;(check-equal? (B-INV-a^nb^n* '(a a b) '(a Z)) #t)
;(check-equal? (B-INV-a^nb^n* '(a a a b b b a a) '(a a Z)) #t)
;(check-equal? (B-INV-a^nb^n* '(b) '(a)) #f)
;(check-equal? (B-INV-a^nb^n* '(a a b b) '(a)) #f)
;(check-equal? (B-INV-a^nb^n* '(a a) '(a)) #f)

;; below is the buggy b-inv
...
(define 2wcw^r (make-ndpda
               ...
               `(((S ,EMP ,EMP) (P ,EMP))
                 ...
                 ((Q ,EMP ,EMP) (F ,EMP)))))
;; Tests for wcw^r
;(check-reject? 2wcw^r '(a) '(a c) '(b c a) '(a a b c b a b))
;(check-accept? 2wcw^r '(c) '(a c a) '(a b b b c b b b a))

;; word stack → Boolean
;; Purpose: Determine in the given word and stack are empty
(define (2S-INV ci s) (and (empty? ci) (empty? s)))
;; Tests for S-INV
;(check-equal? (2S-INV '() '(a a)) #f)
;(check-equal? (2S-INV '(a c a) '()) #f)
;(check-equal? (2S-INV '(a c a) '(b b)) #f)
;(check-equal? (2S-INV '() '()) #t)
...
;; Tests for P-INV
;(check-equal? (2P-INV '(a c a) '(a c a)) #f)
;(check-equal? (2P-INV '(a a) '(a b)) #f)
;(check-equal? (2P-INV '() '()) #t)
;(check-equal? (2P-INV '(a b) '(b a)) #t)
;(check-equal? (2P-INV '(a b a a) '(a a b a)) #t)
...
(define (2Q-INV ci s)
  (let* [(w (takef ci (λ (s) (not (eq? s 'c)))))
    ...
    (and (equal? ci (append w (list 'c) v))
         (equal? w (append (reverse s) (reverse v))))))
;; Tests for Q-INV
;(check-equal? (2Q-INV '(a a) '()) #f)
;(check-equal? (2Q-INV '(b b c a) '(b a)) #f)
;(check-equal? (2Q-INV '(c) '()) #t)
;(check-equal? (2Q-INV '(b a c) '(a b)) #t)
;(check-equal? (2Q-INV '(a b c b) '(a)) #t)
;(check-equal? (2Q-INV '(a b b c b) '(b a)) #t)

;; word stack → Boolean
;; Purpose: Determine if ci=s^Rv^Rcv AND stack is empty
(define (2F-INV ci s)
  (let* [(w (takef ci (λ (s) (not (eq? s 'b)))))]
    (and (empty? s)
         (equal? ci (append w (list 'c) (reverse w))))))
;; Tests for F-INV
;;(check-equal? (2F-INV '() '()) #f)
;;(check-equal? (2F-INV '(b b) '()) #f)
;;(check-equal? (2F-INV '(b a c) '(b a)) #f)
;;(check-equal? (2F-INV '(c) '()) #t)
;;(check-equal? (2F-INV '(b a c a b) '()) #t)
;;(check-equal? (2F-INV '(a b b c b b a) '()) #t)
