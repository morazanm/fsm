...
(define (check-length upper-list
                      ...
                      type
                      check-delta)
  (define upper-bool (if (list? upper-list) ""
  ...
  (define delta-bool
    ...
    ;(if (null? delta) (format "The given list of rules is empty ~s: " delta)
    (if (list? delta)
        ;otherwise check the rules for correct lengths
        (let [(invalid-rules (check-delta))]
          ;if we returned a string, than we have an error
          (if (null? invalid-rules) ""
...
(define (get-repeats a-list)
  ;; find-repeats: list-of-strings list -> list
  ;; Purpose: Given list-of-strings will return a new list that contians all the elements in the origional list that have repeats
  (define (find-repeats los accum)
    (cond
      [(null? los) (remove-duplicates accum)]
...
(define (check-nondependent upper-list sigma lower-name upper-name delta)
                    ...
                    (if (symbol? x)
                        (and (or (char-alphabetic? (string-ref (symbol->string x) 0))
                             ...
                             (or (pred (string-ref (symbol->string x) 0))
                                 (and (= (string-length (symbol->string x)) 2)
  ...
  (define (check-upper)
    (define errors (check-list upper-list
                               char-upper-case? 
                               lower-name
                               #f))
    (if (string=? errors "") ""
  ...
  (define (check-sigma)
    (define errors (check-list (filter (lambda (s) (not (eq? s LM))) sigma)
...
(define (check-nondependent-m delta finals type)
  ...
  (define (repeat-dfa a-list)
    (define (inner los accum f-accum)
      (cond [(null? los) f-accum]
...
(define (check-dependent v a s d t name . f)
  ...
  (define rule-message (if (null? rules)
                           (begin
                             ;(newline)
...
(define (check-machine states sigma finals delta start type . gamma)
  ...
  (define (delta-check-m)
    ;check-lengths: (listof something) num num --> string
    ...
    (define (check-lengths del islist how-many how-long)
      ...
      (define (inner delt accum)
        ;(displayln delt)
        ;check-length: something --> boolean
          ...
          (cond [(and (null? rule)
                ...
                [(= (length (car rule))
                    (car how-long)) (check-length (cdr rule)
                                                  ...
                                                  (cdr how-long))]
                [else #f]))
        (cond [(null? delt) accum] ;if we've reached the end, return accum
    ...
    (if (list? delta)
        ...
        ; correct format
        (cond [(eq? type 'dfa) (check-lengths delta #f 3 '())]
              [(eq? type 'ndfa) (check-lengths delta #f 3 '())]
              [(eq? type 'pda) (check-lengths delta #t 2 (list 3 2))]
              [(eq? type 'tm) (check-lengths delta #t 2 (list 2 2))]
              [(eq? type 'ctm) (error "not yet implemented")])
  ...
  (define big-length-error
    ...
    (string-append (if (string=? (car  length-errors) "") ""
                       (string-append "\n" (car  length-errors)))
                   (if (string=? (cadr length-errors) "") ""
                       (string-append "\n" (cadr length-errors)))
                   (if (string=? (caddr length-errors) "") ""
                       (string-append "\n" (caddr length-errors)))
                   (if (string=? final-bool "") ""
                       (string-append "\n" final-bool))
                   (if (string=? (cadddr length-errors) "") ""
                       (string-append "\nErrors for given rule list: \n" (cadddr length-errors)))))
  ...
  (cond [(not (string=? big-length-error "")) (display big-length-error)]
        ...
        [else
         (begin
           ...
           (define non-dep-errors (if (null? gamma)
           ...
           (cond [(not (string=? non-dep-errors "")) (display non-dep-errors)]
                 ...
                 [else (begin ;(newline)
                         ...
                         (define dep-errors (check-dependent states
  ...
  (cond [(not (string=? big-length-error "")) (display big-length-error)]
        ...
        [else
         (begin
           ...
           ;(newline)
           ;check for nondependent errors
           (define non-dep-errors  (check-nondependent nts
                                                       sigma
                                                       "list of nonterminals" 
                                                       "NONTERMINAL"
                                                       delta))
           ...
           (cond [(not (string=? non-dep-errors "")) (display non-dep-errors)]
                 ...
                 [else (begin ;(newline)
                         ;(display "Nondependent components look good!")
                         (define dep-errors (check-dependent nts
                                                             sigma
                                                             start
                                                             delta
                                                             type
