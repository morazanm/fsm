...
(define-syntax (create-failure-str stx)
  (syntax-parse stx
    [(_ error-type (~var fsm-value) invalid-exprs)
     ...
     (format-id #'error-type "named-single-failure-~a" (syntax-e #'error-type))
     #:with named-multi-failure (format-id #'error-type "named-multi-failure-~a" (syntax-e #'error-type))
     #:with anonymous-single-failure (format-id #'error-type "anonymous-single-failure-~a" (syntax-e #'error-type))
     #:with anonymous-multi-failure (format-id #'error-type "anonymous-multi-failure-~a" (syntax-e #'error-type))
     #'(if (identifier? fsm-value)
           (if (= (length invalid-exprs) 1)
               (named-single-failure (syntax-e fsm-value) (first invalid-exprs))
               (named-multi-failure (syntax-e fsm-value) invalid-exprs))
           (if (= (length invalid-exprs) 1)
               (anonymous-single-failure (first invalid-exprs))
               (anonymous-multi-failure invalid-exprs)))]
    [(_ error-type (~var fsm-value))
     #:with named (format-id #'error-type
                             "named-~a"
                             (syntax-e #'error-type))
     #:with anonymous (format-id #'error-type "anonymous-~a" (syntax-e #'error-type))
     #'(if (identifier? #'fsm-value)
           (named (syntax-e #'fsm-value)
                  (whatami? fsm-value))
           (anonymous (whatami? fsm-value)))]))