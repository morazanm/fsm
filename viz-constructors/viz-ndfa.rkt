...
(define (create-draw-informative-message imsg-st)
  (let* [;;boolean
        ...
        (completed-config? (ormap (λ (config) (empty? (last (last config))))
         ...
         (starting-configs (map (λ (state) (append (list state) (list (append (imsg-state-pci imsg-st)
        ...
        ;;(listof configurations)
         ...
         (curr-config (append-map (λ (config)
                                    (filter (λ (configs)
                                              (equal? (second configs) (imsg-state-upci imsg-st)))
                                            config))
         ...
         (prev-config (cond [(empty? (imsg-state-pci imsg-st)) '()]
                            [(= (length (imsg-state-pci imsg-st)) 1)
                             starting-configs]
         ...
         (current-rules (if (empty? (imsg-state-pci imsg-st))
                            (get-empties-from-start (sm-start (imsg-state-M imsg-st))
                                                 (sm-rules (imsg-state-M imsg-st)))
                            (append-map (λ (path)
                                          (attach-empties path (sm-rules (imsg-state-M imsg-st))))
    ...
    (above/align 'left
                 (cond [(and (empty? (imsg-state-pci imsg-st))
                        ...
                        (above/align 'left
                                     (beside (text "aaaa" 20 'white)
                                             (text "Word: " 20 'black)
                                             (if (equal? (sm-apply (imsg-state-M imsg-st)
                                                                   (imsg-state-pci imsg-st)) 'accept)
                                                 (text (format "~a" EMP)
                                                       20
                                                       'gray)
                                                 (text (format "~a" EMP)
                       ...
                       [(empty? (imsg-state-upci imsg-st))
                        (above/align 'left
                                     (beside (text "aaaa" 20 'white)
                                             (text "Word: " 20 'black)
                                             (text (los2str (imsg-state-pci imsg-st))
                                                   20
                                                   'gray))
                                     (beside (text "Consumed: " 20 'black)
                                             (text (los2str (imsg-state-pci imsg-st))
                       ...
                       [(empty? (imsg-state-pci imsg-st))
                        (above/align 'left (beside (text "aaaa" 20 'white)
                                                   (text "Word: " 20 'black)
                                                   (text (los2str (imsg-state-upci imsg-st))
                                                         20
                                                         'black))
                       ...
                       [else (above/align 'left
                                          (beside (text "aaaa" 20 'white)
                                                  (text "Word: " 20 'black)
                                                  (beside (text (los2str (imsg-state-pci imsg-st))
                                                                20
                                                                'gray)
                                                          (text (los2str (imsg-state-upci imsg-st))
                                                                20
                 ...
                 (beside (text (format "The number of possible computations is ~a. "
                         ...
                         (cond [(and (empty? (imsg-state-upci imsg-st))
                                     (equal? (sm-apply (imsg-state-M imsg-st)
                                                       (imsg-state-pci imsg-st)) 'accept))
...
(define (create-graph-thunk a-vs)
  (let* [ ;;for reference, a config(computation) is a pair(list) with a state as the first element and a word as the second
         ...
         ;;(listof configurations)
         ;;Purpose: Returns all starting configurations of for the entire word
         (starting-configs (map (λ (state) (append (list state) (list (append (building-viz-state-pci a-vs)
                                                                              (building-viz-state-upci a-vs)))))
                                (cons (sm-start (building-viz-state-M a-vs))
                                      (get-empty-states-from-start (sm-start (building-viz-state-M a-vs))
         ...
         (curr-config (append-map (λ (config)
                                    (filter (λ (configs)
                                              (equal? (second configs) (building-viz-state-upci a-vs)))
                                            config))
         ...
         ;;(listof configurations)
         ;;Purpose: Returns the all configurations using the CI that is one step behind
         (prev-config (cond [(empty? (building-viz-state-pci a-vs)) '()]
                            [(= (length (building-viz-state-pci a-vs)) 1)
                             starting-configs]
                            [else (append-map (λ (config)
                                                (filter (λ (configs)
                                                          (equal? (second configs)
         ...
         ;;     the third of the rule, and the first of letter in the word of the previous
         ;;     config with the second of the rule to obtain the current rule(s) being applied 
         (current-rules (if (empty? (building-viz-state-pci a-vs))
                            (get-empties-from-start (sm-start (building-viz-state-M a-vs))
                                                    (sm-rules (building-viz-state-M a-vs)))
                            (append-map (λ (path)
                                          (attach-empties path (sm-rules (building-viz-state-M a-vs))))
                                        (remove-duplicates (for*/list ([prev prev-config]
                                                                       [curr curr-config]
...
(define (create-graph-thunks a-vs acc)
  (cond [(empty? (building-viz-state-upci a-vs))
        ...
        [else (let [(next-graph (create-graph-thunk a-vs))]
                (create-graph-thunks (struct-copy building-viz-state a-vs
...
(define (right-key-pressed a-vs)
  (let* [;;boolean
         ;;Purpose: Determines if the pci can be can be fully consumed
         (completed-config? (ormap (λ (config) (empty? (last (last config))))
                                   (get-configs (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)));(viz-state-ndfa-pci a-vs)
                                                (sm-rules (imsg-state-M (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                (sm-start (imsg-state-M (informative-messages-component-state (viz-state-informative-messages a-vs)))))))
         (test0 (displayln (informative-messages-component-state (viz-state-informative-messages a-vs))))]
    (struct-copy viz-state a-vs
                 [informative-messages
                  (struct-copy informative-messages (viz-state-informative-messages a-vs)
                               [component-state (struct-copy imsg-state (informative-messages-component-state (viz-state-informative-messages a-vs))
                                            [upci (if (or (empty? (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                          (not completed-config?))
                                                      (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                      (rest (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))))]
                                            [pci (if (or (empty? (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                         (not completed-config?))
                                                     (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                     (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                             (list (first (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))))))]
                                            )
                                                ]
                               )
                  ]
...
(define (down-key-pressed a-vs)
  (let* [;;(listof symbols)
         ;;Purpose: The last word that could be fully consumed by the ndfa
         (last-consumed-word (last-fully-consumed (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                          (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                  (imsg-state-M (informative-messages-component-state (viz-state-informative-messages a-vs)))))
         ;;(listof symbols)
         ;;Purpose: The portion of the word that cannont be consumed
         (unconsumed-word (remove-similarities last-consumed-word
                                               (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
    ...
    (struct-copy viz-state a-vs
                 [informative-messages
                  (struct-copy informative-messages (viz-state-informative-messages a-vs)
                               [component-state (struct-copy imsg-state (informative-messages-component-state (viz-state-informative-messages a-vs))
                                                             [upci (cond [(empty? (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                          (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))]
                                                                         [(not (equal? last-consumed-word (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                                                                  (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))))
                                                                          (rest unconsumed-word)]
                                                                         [else '()])]
                                                             [pci (cond [(empty? (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                         (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))]
                                                                        [(not (equal? last-consumed-word (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                                                                 (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))))
                                                                         (append last-consumed-word (take unconsumed-word 1))]
                                                                        [else (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                                      (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))])]
                                                             )
                                                ]
                               )
...
(define (left-key-pressed a-vs)
  (struct-copy viz-state a-vs
               [informative-messages
                (struct-copy informative-messages (viz-state-informative-messages a-vs)
                             [component-state (struct-copy imsg-state (informative-messages-component-state (viz-state-informative-messages a-vs))
                                                           [upci (if (empty? (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                     (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                     (cons (last (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                           (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))))]
                                                           [pci (if (empty? (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                    (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                    (take (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                          (sub1 (length (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))))))]
                                                           )
                                              ]
                             )
                ]))
...
(define (up-key-pressed a-vs)
  (struct-copy viz-state a-vs
               [informative-messages
                (struct-copy informative-messages (viz-state-informative-messages a-vs)
                             [component-state (struct-copy imsg-state (informative-messages-component-state (viz-state-informative-messages a-vs))
                                                           [upci (if (empty? (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs))))
                                                                     (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                     (append (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs)))
                                                                             (imsg-state-upci (informative-messages-component-state (viz-state-informative-messages a-vs)))))]
                                                           [pci (if (empty? (imsg-state-pci (informative-messages-component-state (viz-state-informative-messages a-vs))))
      ...
      (let* [(new-M (make-new-M M))
             (dead-state (cond [(and add-dead
                         ...
                         [(and add-dead
                          ...
                          DEAD]
                         [else 'no-dead]))
             (graphs (create-graph-thunks (building-viz-state a-word
                                '()
                                (if add-dead
                                    new-M
                                    M)
        ...
        (run-viz graphs
                 ...
                 DEFAULT-ZOOM-FLOOR
                 (informative-messages
                  create-draw-informative-message
