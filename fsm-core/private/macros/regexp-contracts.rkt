                      ...
                      (let [(extra-singletons (find-extra-singleton-regexps regexp sigma))]
                        (if (empty? extra-singletons)
                            ...
                            (raise-blame-error
                             blame
                             sigma
                             (format "Step one of the design recipe has not been successfully completed.\nThe following singltons: ~a are in the regexp, but not in the specified alphabet"
                                     extra-singletons))))
                      ))))
                    ...
                    (lambda (x)
                      ...
                      (if (boolean? x)
                          ...
                          (raise-blame-error
                           blame
                           x
                           (format "Step three of the design recipe has not been successfully completed.\nThe output to a regexp predicate should be a boolean, but found")))))))
                        ...
                        (let [(failing-words (filter (lambda (word) (not (predicate word))) words))]
                          (if (empty? failing-words)
                              ...
                              (raise-blame-error
                               blame
                               failing-words
                               (format "Step three of the design recipe has not been successfully completed.\nThe specified predicate determined the following words are not in the language of the regexp when they are"))))
                        )))))
    ...
    (->i ([r1 (valid-regexp/c "The first argument to concat-regexp must be a regular expression, but found")]
         ...
         (#:pred [pred (r1 r2) (and/c
          ...
          #:accepts [accepts (r1 r2) (and/c
                                      (listof-words/c "accepts" "four")
                                      (regexp-input/c (make-unchecked-concat r1 r2)
                                                      'concat-regexp
                                                      #t))]
          #:rejects [rejects (r1 r2) (and/c
                                      (listof-words/c "rejects" "four")
                                      (regexp-input/c (make-unchecked-concat r1 r2)
                                                      'concat-regexp
                                                      #f))]
          #:sigma [sigma (r1 r2) (and/c (is-a-list/c "regexp alphabet" "one")
