...
;; state (listof pda-rule) -> (listof pda-rule)
;; Purpose: To return the next pda-rules that can be used from the given pda-rules
(define (get-next-rules a-state rules-ht)
  (hash-ref rules-ht a-state))
...
(define (new-paths&visited a-path rules visited max-length)
  ...
  (define (new-paths-helper next-rules accum #;new-visited)
    (cond [(empty? next-rules) (list accum visited #;new-visited)]
          ...
          [else (let* [(new-path-stack (append (if (eq? 'ε (get-push (first next-rules)))
                                                   '()
                       ...
                       (new-config (make-config (get-destination-state (first next-rules))
                                                new-path-word ;<- the word is in reverse
                                                new-path-stack))
                       
                       (new-path-rules (cons new-config ;<- extending the path of configs with the new config ;<- the paths are in reverse, these paths are a list of configs of state sv
                                             (PATH-lor a-path)))]
...
(define (find-paths-pda a-machine #:max-length [max-length 12])
    ...
    (if (queue-empty? queue)
        ...
        (let* [(qfirst (dequeue! queue))
               ...
               (paths-with-qfirst (if (and (empty? (PATH-stack qfirst))  ;<- only accumulating the accepting paths, no subpaths
                                      ...
                                      (cons (make-PATH (reverse (PATH-lor qfirst))  ;<- also have to unreverse the configs
                                                       (PATH-stack qfirst)
  ...
  (begin
    (for/list ([i rules]
      ...
      (let [(path-word (if (eq? 'ε (get-elem-read i))
                                     '()                                 ;; <- the current word of the path 
                                     (list (get-elem-read i))))
            (path-stack (if (eq? 'ε (get-push i))
                                     '()                   ;;  ;; <- current stack of the path
                                     (get-push i)))
            (path-dest-state (get-destination-state i))]
        (enqueue! queue (make-PATH (list (make-config path-dest-state path-word path-stack)) ;<- current configs of the path      ;;  ⚙️making/adding the first paths to the queue
                                   path-stack 
                                   path-word
...
;; pda natnum -> (listof configs)
;; Purpose: returns the all the accepting configurations that lead to an accepting word of the given machine
(define (get-accepting-configs a-pda max-length)
  (define paths-that-end-in-finals (find-paths-pda a-pda #:max-length max-length))

  (define config-set (mutable-set))
  

  (for ([path (in-list paths-that-end-in-finals)])
    (for ([path-config (in-list (PATH-lor path))])  ;<- adding all the configs to the set to remove duplicates
      (set-add! config-set path-config)))
...
(define (path-inv-not-hold? a-path a-loi)
  (not (invariant-holds? a-loi
                         (reverse (config-rev-word a-path))
                         (config-stack a-path))))
...
(define (sm-test-invs-pda a-machine max-path-length ds-remove? a-loi)
  ...
  (define all-paths-new-machine (get-accepting-configs new-machine max-path-length))
