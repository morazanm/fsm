  ...
  (let (
    ...
    (beside/align "bottom"
                  ...
                  (beside (above/align "right"
                          ...
                          (above/align "left"
                                       (text "Zoom in" 18 'black)
                                       (square 10 'solid 'white)
                                       (text "Zoom out" 18 'black)
                                       (square 10 'solid 'white)
                                       (text "Reset the viewport" 18 'black)
                                       )
                          )
         ...
         (let  [
           ...
           (if (> (viz-state-scale-factor-cap a-vs) new-scale)
               (let* [
                      ...
                      (scaled-image (if (or (< E-SCENE-WIDTH (image-width (first (viz-state-pimgs a-vs))))
                                            (< E-SCENE-HEIGHT (image-height (first (viz-state-pimgs a-vs))))
                                            )
                                        (scale new-scale (first img-resize))
                                        (scale new-scale (first (viz-state-pimgs a-vs)))
                                        )
                                    )
                      ...
                      (MAX-X (if (< E-SCENE-WIDTH (/ (image-width scaled-image) 2))
                                 (+ (- E-SCENE-WIDTH (/ (image-width scaled-image) 2)) E-SCENE-WIDTH)
                                 E-SCENE-WIDTH
                                 )
                             )
                      ...
                      (MAX-Y (if (< E-SCENE-HEIGHT (/ (image-height scaled-image) 2))
                                 (+ (- (/ (image-height scaled-image) 2) E-SCENE-HEIGHT) E-SCENE-HEIGHT)
                                 (+ (/ (image-height scaled-image) 2) tools-size)
                                 )
                             )
                      ]
                 (cond [(and (> MIN-X (posn-x (viz-state-image-posn a-vs)))
                       ...
                       ;[else (error (format "~s ~s ~s ~s" MIN-X MAX-X MIN-Y MAX-Y))]
                       )
                 )
               a-vs
               )
