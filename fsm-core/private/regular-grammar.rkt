...
(define (rg-derive-with-rules g w)
    ...
    (cond [(null? tovisit) null]
          [else (let* ((firstpath (car tovisit))
                  ...
                  (cond [(equal? (first curr) w) firstpath]
                        [else (let* ((new-words (apply-one-step (first curr) (rg-getrules g)))
                                     (newstrings (if (null? new-words) 
                                                   null
  ...
  (define (convert-rule-struct-to-list rule-struct)
    (cond [(erule? rule-struct) (list (erule-lhs rule-struct) ARROW EMP)]
