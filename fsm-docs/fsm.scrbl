...
@defproc[(sm-test-invs [m machine?] [#:rep-limit rep-limit natural? 1] [#:max-path-length path-length natural? 15] [#:ds-remove-fsa ds-remove-fsa #t] [#:ds-remove-pda ds-remove-pda #f] [invs (list state (-> word boolean))] ...)
         (listof (list state (list word)))]
Tests the given machine using the given state invariants. Dead states are removed for finite state machines for testing purposes, @italic{ds-remove-fsa}
is used to determine if they are removed. For pushdown automata, @italic{ds-remove-pda} is used to determine if dead states are removed.
If m is a finite-state machine, then @italic{rep-limit} is used to
...
Otherwise, the function returns a list of pairs, where each pair contains a state and the words for which the invariant predicate has failed.
...
@defproc[(grammar-derive? [g grammar] [w word])
         Boolean]{Determines if the given word is in the language of the given grammar.

 Note: For a given unrestricted grammar, the derivation search may not terminate.}
...
@defproc[(grammar-derive [g grammar] [w word])
 ...
 Note: For a given unrestricted grammar, the derivation search may not
 terminate.}
