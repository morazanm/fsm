...
(define ((compressed-vec-ref bit-width mask) vec idx) 
  ...
  (cond [(fx<= (fx+ bit-offset bit-width) num-bits)
         (fxand (fxrshift/logical (fxvector-ref (unsafe-struct*-ref vec 0) word-idx) bit-offset) mask)]
        [else
         (fxand (fxior (fxrshift/logical (fxvector-ref (unsafe-struct*-ref vec 0) word-idx) bit-offset)
                       (fxlshift/wraparound (fxvector-ref (unsafe-struct*-ref vec 0) (fx+ word-idx 1)) (fx- num-bits bit-offset)))
                mask)]))
...
(define ((compressed-vec-set! bit-width mask) vec idx val)
  (define-values (word-idx bit-offset) (quotient/remainder (fx* bit-width idx) num-bits))
  (cond [(fx<= (fx+ bit-offset bit-width) num-bits)
         (fxvector-set! (unsafe-struct*-ref vec 0)
                        word-idx
                        (fxior (fxand (fxvector-ref (unsafe-struct*-ref vec 0) word-idx)
        ...
        [else
         ...
         (fxvector-set! (unsafe-struct*-ref vec 0)
                        new-idx
                        (fxior (fxand (fxvector-ref (unsafe-struct*-ref vec 0) new-idx)
...
(define (create-gcfxv-functions bit-width)
  ...
  (define (gcfxvector-ref gv index)
    (g-compressed-vec-ref (unsafe-struct*-ref gv 0) index))

  
  (define (ensure-free-space-gcfxvec! gcfxv)
    (define-values (nums-allocated size) (g-compute-size-values (fx* 2 (unsafe-struct*-ref (unsafe-struct*-ref gcfxv 0) 1))))
    (define new-vec (make-fxvector nums-allocated 0))
    (for ([idx (in-range (fxvector-length (unsafe-struct*-ref (unsafe-struct*-ref gcfxv 0) 0)))])
      (fxvector-set! new-vec idx (fxvector-ref (unsafe-struct*-ref (unsafe-struct*-ref gcfxv 0) 0) idx)))
    (unsafe-struct*-set! (unsafe-struct*-ref gcfxv 0) 0 new-vec)
    (unsafe-struct*-set! (unsafe-struct*-ref gcfxv 0) 1 size))
  
  
  (define (gcfxvector-add! gv item)
    (when (fx>= (unsafe-struct*-ref gv 1) (unsafe-struct*-ref (unsafe-struct*-ref gv 0) 1))
      (ensure-free-space-gcfxvec! gv))
    (g-compressed-vec-set! (unsafe-struct*-ref gv 0) (unsafe-struct*-ref gv 1) item)
    (unsafe-struct*-set! gv 1 (fx+ (unsafe-struct*-ref gv 1) 1)))

  
  (define (gcfxvector-add/copy gv item)
    (define new-vec
      (gcfxvector (compressedvec (fxvector-copy (unsafe-struct*-ref (unsafe-struct*-ref gv 0) 0))
                                 (unsafe-struct*-ref (unsafe-struct*-ref gv 0) 1))
                  (unsafe-struct*-ref gv 1)))
