...
(module fsa racket
  ...
  (provide make-dfa make-ndfa union-fsa concat-fsa kleenestar-fsa complement-fsa intersection-fsa
  ...
  (define (ndfa->dfa m . L)
    (define (ndfsm->dfsm m)
      (let* ((esr (compute-superstate-rules null (list (sort-symbols (empties (fsa-getstart m) (fsa-getrules m)))) (fsa-getalphabet m) (fsa-getrules m) null))
        ...
        (if (null? L)
            (make-dfa (map superstate->state new-states) (fsa-getalphabet m) new-start new-finals new-rules)
            (make-dfa (map superstate->state new-states) (fsa-getalphabet m) new-start new-finals new-rules (car L)))))
    (ndfsm->dfsm m))
  ...
  (define (rename-states-fsa los m)
    (let* ((mstates (fsa-getstates m))
           ...
           (new-rules (map (lambda (r) (list (cadr (assoc (from-state-fsarule r) rename-table))
                           ...
                           (m null 'get-deltas))))
      (cond [(m '() 'is-deterministic?) (make-dfa new-states (m null 'get-sigma) new-start new-finals new-rules 'nodead)]
            [else (make-ndfa new-states (m null 'get-sigma) new-start new-finals new-rules 'nodead)])))
  ...
  (define (concat-fsa m1 m2)
    (let* ((nm1 (rename-states-fsa (fsa-getstates m2) m1))
           (nm2 (rename-states-fsa (fsa-getstates nm1) m2)))
      (make-ndfa (union-states (fsa-getstates nm1) (fsa-getstates nm2)) ; nm1 & nm2 have no common state names
  ...
  (define (kleenestar-fsa m1)
    (let ((nm1 (rename-states-fsa (list START) m1))) ; guarantees no conflict with START
      (make-ndfa (cons START (fsa-getstates nm1))
                 (nm1 null 'get-sigma)
                 START
                 (cons START (fsa-getfinals nm1))
                 (cons (mk-fsarule START EMP (fsa-getstart nm1))
                       (append (fsa-getrules nm1)
                               (map (lambda (s) (mk-fsarule s EMP (fsa-getstart nm1))) 
                                    (fsa-getfinals nm1)))))))
  ...
  (define (fsa->regexp m)
    ...
    (define (build-reg-exp table finals rules n start)
      ...
      (define (make-lunion-regexp l)
        (let ((car-regexp (if (eq? (car l) EMP) (empty-regexp) (singleton-regexp (symbol->string (car l))))))
          (cond [(null? (cdr l)) car-regexp]
                [else (union-regexp car-regexp
        ...
        (cond [(= k 0)
               (let* ((rls (filter (lambda (r) (and (eq? (from-state-fsarule r) (cadr (assoc i table)))
                 ...
                 (let* ((x (cond [(= (length rls) 0) (null-regexp)]
                   ...
                   (let* ((ans (if (= i j) (union-regexp (empty-regexp) x) x)))
                     ans)))]
              [else (let* ((R1 (R i j (- k 1)))
                      ...
                      (union-regexp R1
                                    (concat-regexp R2
                                                   (concat-regexp (kleenestar-regexp R3) R4))))]))
      (cond [(null? finals) (null-regexp)]
            ...
            [else (let* ((current-final (car finals)))
                    (union-regexp (R 1 (get-assoc current-final table) n)
  ...
  (define (regexp->fsa r)
    ...
    (define (build-fsa r)
      ...
      (define (make-empty-fsa)
        (make-dfa '(q0) SIGMA 'q0 '(q0) '()))
      ; symbol --> fsa
      (define (make-singleton-fsa symb)
        (make-dfa '(q0 q1) SIGMA 'q0 '(q1) (list (mk-fsarule 'q0 symb 'q1))))
  ...
  (define (fsa->rg m)
    ...
    (let* ((dm (if (is-deterministic-fsa? m) m (ndfa->dfa m)))
      ...
      (make-rg V (fsa-getalphabet dm) (append rls1 rls2 rls3) (symbol-upcase (fsa-getstart dm)))))
  ...
  (define (rg->fsa g)
    ...
    (let ((F (list 'QF))
      ...
      (make-ndfa K SIGMA S F DELTAS 'nodead))) ; ok not to add the dead state?
