...
(begin-for-syntax
  ...
  ;This takes in two lists of symbol syntax of length two or more and returns
  ; whether or not those lists are identical
  ...
  ;This checks to see if the list of symbol syntax of length 2 or more exists within
  ; the list of (symbol syntaxes of length two or more) and returns a boolean
  (define (rule-member? rule rule-list)
    (ormap (lambda (x) (compare-rule-start rule x)) rule-list))

  ;takes in a list of rules which are lists of symbol syntax. It grabs the starting
  ; elements of each list, and then sees if those exist more than once in the rule set.
  ; If there are none that repeat, it does not fail, and it returns false, otherwise
  ; it returns the duplicating syntax.
  ...
  ;takes in a list of rules that have each symbol in sytax, and a list of the first
  ; two elements of each rule as generated by the cartisean product function.
  ; After that, it takes the first two elements of each rule from the rule list and
  ; makes a list. Then it checks over every element in the pair-list, sees if it
  ; exists inside the rules, and adds them to a leftovers list if they're not included
  ; If that list is empty, it is not failing, so it returns false. Otherwise, it
  ; retuns the leftovers since that set of rules would not be functional.
  ...
  ;Takes in a syntax of a list of symbols for the start states, alphabets, and end states
  ; of each rule. Converts them to lists of sytax, passes them to a map which rebuilds
  ; the rules in an actual list of sytax, and gives it back
