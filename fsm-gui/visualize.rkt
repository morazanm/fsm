      ...
      (
       ...
       (determim-prev-rule (lambda (rule)
                             (let ((c-rule (if (equal? MACHINE-TYPE 'pda)
                                               (getCurRule rule (machine-rule-list (world-fsm-machine w)) (pda-transitions-with-rules ORIGINAL-MACHINE TM-ORIGIONAL-TAPE (machine-start-state (world-fsm-machine w))))
