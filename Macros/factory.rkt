#lang racket
(require (for-syntax racket/syntax syntax/stx syntax/parse racket/list racket/string racket/match))
...
(begin-for-syntax
  ...
  ;;   output: [(list (list _ a-1 _) (list a-2 _)) #:when (and (number? a-1) (symbol? a-2)) (pda-rule-to-string data)]
  (define (trans-row stx-patt stx-func)
    (syntax-parse stx-patt
      [((_ ...)...) (let* [(f (stx-map (λ (inner) (trans-case (syntax->list inner))) stx-patt))
                      ...
                      #`[(list #,@t) #:when (and #,@g) (#,stx-func data)])]
      [(_ ...) (match-let [(`(,p ,g) (trans-case (syntax->list stx-patt)))]
                 #`[(list #,@p) #:when (and #,@g) (#,stx-func data)])])))
